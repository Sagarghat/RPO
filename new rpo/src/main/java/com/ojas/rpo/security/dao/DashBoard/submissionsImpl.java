...
public class submissionsImpl extends JpaDao<Submissions, Long> {
	...
	public List getData1(Long requirementID) {
		Query q = getEntityManager().createNativeQuery(" SELECT  COUNT(*)  noofprofilessummited,"
				+ "bdmreq.id requirementId," + "assign.target," + "bdmreq.nameOfRequirement,"
				+ "candidate.candidateStatus, " + "USER.name, " + " clients.clientName " + " FROM   bdmreq  bdmreq, "
				+ " candidate  candidate, " + " assign assign , " + " user USER, " + " recuritermap recuritermap, "
				+ " CLIENT clients " + " WHERE bdmreq.id=candidate.bdmReq_id " + "  AND  bdmreq.id=?"
				+ " AND assign.bdmReq_id=bdmreq.id " + " AND recuritermap.recuriter_ID=USER.id "
				+ " AND assign.id=recuritermap.Assign_ID " + "AND candidate.user_id=recuritermap.recuriter_ID"
	...
	public List getData(Long requriterID) {
		Query q = getEntityManager().createNativeQuery(" SELECT  COUNT(*)  noofprofilessummited,"
				...
				+ "candidate.candidateStatus, " + "USER.name, " + " clients.clientName " + " FROM   bdmreq  bdmreq, "
				+ " candidate  candidate, " + " assign assign , " + " USER USER, " + " recuritermap recuritermap, "
				+ " CLIENT clients " + " WHERE bdmreq.id=candidate.bdmReq_id " + "  AND   USER.id=?"
				+ " AND assign.bdmReq_id=bdmreq.id " + " AND recuritermap.recuriter_ID=USER.id "
				+ " AND assign.id=recuritermap.Assign_ID " + "AND candidate.user_id=recuritermap.recuriter_ID"
				+ " AND clients.id = bdmreq.client_id GROUP BY " + " bdmreq.id ");
