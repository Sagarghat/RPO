		...
		.controller(
				...
				function($scope, $routeParams, $rootScope, $location, $http,
					...
					$scope.getActiveContacts = function(id) {
						...
						var promise = bdmService.clientcontactactive(id, role,
								userId);
						promise.then(function(response) {
							$scope.chiru = response.data.result;
							console.log($scope.chiru);
					...
					$scope.getRequirementContact = function(addContact1) {
						 ...
						 * $scope.email = jhansi.email; $scope.mobile =
						 * jhansi.mobile; $scope.contact = jhansi.id;
						 */
					...
					$scope.save = function() {
						...
						angular.forEach($scope.skillsList, function(skills) {
							if (skills.selected)
								$scope.skillsArray.push({
									id : skills.id,
									skillName : skills.skillName
								});
						});
						console.log($scope.skillsArray);
					}
					...
					$scope.searchQualification = function(qualification) {
						...
						$scope.isLoading = true;
						$http.get(
								"rest/Bdmrequire/getqualification/" + $scope.pageIndex
										+ "/" + $scope.pageSize
					...
					$scope.searchLocations  = function(qualification) {
						debugger;
						/*
						 * if ($scope.skill == undefined || $scope.skill == "") {
						 * swal("Please Enter Skill and Search"); return false; }
						...
						$scope.isLoading = true;
						$http.get(
								"rest/Bdmrequire/getlocations/" + $scope.pageIndex
										+ "/" + $scope.pageSize
					...
					$scope.bdmreqdtls = function() {
						...
						if ($scope.status == undefined) {
							...
							return false;
						} else if ($scope.skillsArray.length == 0) {
						...
						$scope.value = {
							...
							relavantExperience : $scope.minrelevant + "-"
									+ $scope.maxrelevant,
							famount : $scope.famount,
							status : $scope.status,
							locations : $scope.locationarray,
