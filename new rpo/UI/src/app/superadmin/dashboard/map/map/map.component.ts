...
export class MapComponent implements OnInit {
  ...
  sendclientmail() {
    ...
    let data = {
      ...
      fileType: this.files[0].name
    }
    this.ser.candidateEmails(data).subscribe(resp => {
      this.submail = resp;
      console.log(this.submail, "submail");

      if (this.submail.status === "StatusSuccess") {
        $("#cliente").modal("hide");
        // this.toaster.successToastr('Email has sent to client successfully');
        this.submitCustomer();
  ...
  submitCustomer() {
    ...
    this.ser.submitcustomer(data).subscribe(resp => {
      ...
      console.log(this.submitClient, "response by submitting to client");
      if (this.submitClient['status'] === 'StatusSuccess') {
        // this.display='none';
        this.toaster.successToastr('submitted to customer');
        this.getscreendetails();
      }
      if (this.submitClient['status'] === 'Bad_Request') {
        this.toaster.warningToastr(this.submitClient['res']);
      }
    });
  }
